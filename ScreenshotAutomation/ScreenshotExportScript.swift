...
let processor = FileManager.default.temporaryDirectory.appendingPathComponent(
    "timely-quote-screenshot-processor"
)
...
try run(
    ...
    [
        "swiftc",
        "-module-cache-path",
        FileManager.default.temporaryDirectory.appendingPathComponent("timely-quote-module-cache")
            .path,
...
let devices: [(rawValue: String, simulator: String)] = [
    (
        "iPhone_6.7",
        ProcessInfo.processInfo.environment["SCREENSHOT_IPHONE_SIMULATOR"] ?? "iPhone 17 Pro Max",
    ),
    (
        "iPad_12.9",
        ProcessInfo.processInfo.environment["SCREENSHOT_IPAD_SIMULATOR"] ?? "iPad Pro 13-inch (M5)",
    ),
]
let screenNames = [
    "01_LiteraryClock", "02_Personalize", "03_DarkMode", "04_ReadTheBook", "05_OpenSource",
]
